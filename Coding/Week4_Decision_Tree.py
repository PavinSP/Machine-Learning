    ...
    def train(self,maxDepth,depth=0):
        ...
        for i in range(self.X.shape[0]):
            for j in range(self.X.shape[1]):
                #TODO 0
                leftIdx = self.X[:,j] < self.X[i,j]
                rightIdx = self.X[:,j] >= self.X[i,j]
                score[i, j] = self.H(self.y) - (np.sum(leftIdx)*self.H(self.y[leftIdx]) + np.sum(rightIdx)*self.H(self.y[rightIdx]))/self.X.shape[0]
                
                #compute score[i,j] (= information gain when splitting at sample i and feature j)
                #Hint: use self.H() to compute the entropy of the parent node and the two child nodes
        #TODO 1
